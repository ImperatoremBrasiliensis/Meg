...
TEST_F(ProtocolsUtilitiesVector, NewReturnsValidPointerAndStoresTypeSizeInTheAllocatedStruct) {
   v = prosVector_new(sizeof(int), nullptr);
...
TEST_F(ProtocolsUtilitiesVector, NewCallWithZeroTypeSizeParameterPanics) {
   EXPECT_DEATH(
      {
         [[maybe_unused]]
         prosVector v = prosVector_new(0, nullptr);
      },
      ""
   );
}
...
TEST_F(ProtocolsUtilitiesVector, DelFreesTheObjectTurnsHandleNullptr) {
   v = prosVector_new(sizeof(int), nullptr);
   prosVector_del(&v);
   EXPECT_EQ(v.capacity, 0);
   EXPECT_EQ(v.size, 0);
   EXPECT_EQ(v.typeSize, 0);
   EXPECT_FALSE(v.data);
   EXPECT_FALSE(v.deleter);
}
...
TEST_F(ProtocolsUtilitiesVector, MetReserveIncreaseCapacityAndPreserveData) {
   ...
   ASSERT_NO_FATAL_FAILURE(prosVector_reserve(&v, 256))
      << "Humm... Something went wrong.";
...
TEST_F(ProtocolsUtilitiesVector, MetReserveCallWithNullSelfParameterPanics) {
   EXPECT_DEATH({ prosVector_reserve(nullptr, 0); }, "");
}
...
TEST_F(ProtocolsUtilitiesVector, MetPushBackCopiesBytesAndDoesNotAliasSourceMemory) {
   v = prosVector_new(sizeof(PaddedObject), nullptr);
   ASSERT_EQ(v.typeSize, sizeof(PaddedObject));

   PaddedObject obj;
   memset(&obj, 0xAf,
      sizeof(obj));  // To `obj` (including padding) contain only known bytes.
   obj = {.a = 0x11u, .b = 0x22334455u, .c = 0x6677u};
   ...
   EXPECT_EQ(
      memcmp(prosVector_getData(&v), expected.data(), sizeof(PaddedObject)),
      0
   );
}
...
TEST_F(ProtocolsUtilitiesVector, MetPushBackCallWithNullSelfParameterPanics) {
   int value = 1;
   EXPECT_DEATH({ prosVector_pushBack(nullptr, &value); }, "");
}

TEST_F(ProtocolsUtilitiesVector, MetPushBackCallWithNullObjParameterPanics) {
   EXPECT_DEATH(
      {
         prosVector vext = prosVector_new(sizeof(int), nullptr);
...
TEST_F(ProtocolsUtilitiesVector, MetPopBackCallWithNullSelfParameterPanics) {
   EXPECT_DEATH({ prosVector_popBack(nullptr); }, "");
}
...
TEST_F(
   ProtocolsUtilitiesVector,
   MetInsertWithSpecificInsertionOrderDoublesCapacityWhileUpdatesSizeAndPreserveTheObjects
) {
   v = prosVector_new(sizeof(int), nullptr);
   ASSERT_EQ(v.typeSize, sizeof(int));
   ...
   EXPECT_EQ(v.size, 5);
   [[maybe_unused]] int (&p)[8] =
      *static_cast<int (*)[8]>(prosVector_getData(&v));
   ...
   for (int i = 0; i < 8; i++)
      EXPECT_EQ(expectedValues[i], *static_cast<int *>(prosVector_getAt(&v, i)))
         << "Iteration: " << i;
...
TEST_F(ProtocolsUtilitiesVector, MetInsertCopiesBytesAndDoesNotAliasSourceMemory) {
   ...
   PaddedObject obj;
   memset(&obj, 0XAF, sizeof(PaddedObject));  // To `obj` (including padding)
                                              // contain only known bytes.
   obj = {.a = 0x11u, .b = 0x22334455u, .c = 0x6677u};
   ...
   EXPECT_EQ(
      memcmp(expected.data(), prosVector_getData(&v), sizeof(PaddedObject)),
      0
   );
}
...
TEST_F(ProtocolsUtilitiesVector, MetInsertIntoAnAlreadyOcuppiedIndexMovesObjects) {
   ...
   for (int i = 0; i < 3; i++)
      EXPECT_EQ(*static_cast<int *>(prosVector_getAt(&v, i)), expectedValues[i])
         << "Iteration: " << i;
}

TEST_F(ProtocolsUtilitiesVector, MetInsertCallWithNullSelfParameterPanic) {
   int vl = 1;
   EXPECT_DEATH({ prosVector_insert(nullptr, &vl, 0); }, "");
}
...
TEST_F(ProtocolsUtilitiesVector, MetRemoveCallWithNullSelfParameterPanics) {
   EXPECT_DEATH({ prosVector_remove(nullptr, 0); }, "");
}

/* Method prosVector_pushArray() */

TEST_F(ProtocolsUtilitiesVector, MetPushArrayAppendsTheObjectsAndUpdatesSizeWhileCapacityHasRoom) {
   v = prosVector_new(sizeof(int), nullptr);
   ASSERT_EQ(v.typeSize, sizeof(int));

   prosVector_reserve(&v, 8);
   ASSERT_EQ(v.capacity, 8);
   ASSERT_EQ(v.size, 0);

   // Array.
   int vl[] = {0, 1, 2, 3, 4, 5, 6, 7};

   prosVector_pushArray(&v, vl, 4);
   ASSERT_EQ(v.capacity, 8);
   ASSERT_EQ(v.size, 4);
   for (int i = 0; i < 4; i++)
      EXPECT_EQ(vl[i], ((int *) prosVector_getAt(&v, i))[0]);

   prosVector_pushArray(&v, &vl[4], 4);
   ASSERT_EQ(v.capacity, 8);
   ASSERT_EQ(v.size, 8);
   for (int i = 0; i < 8; i++)
      EXPECT_EQ(vl[i], ((int *) prosVector_getAt(&v, i))[0]);
...
TEST_F(ProtocolsUtilitiesVector, MetPushArrayIncreasesCapacityWhenTheVectorIsFull) {
   v = prosVector_new(sizeof(int), nullptr);
   ASSERT_EQ(v.typeSize, sizeof(int));

   prosVector_reserve(&v, 2);
   ASSERT_EQ(v.capacity, 2);
   ASSERT_EQ(v.size, 0);

   int vl[] = {0, 1, 2, 3, 4, 5, 6, 7};

   prosVector_pushArray(&v, vl, 2);
   ASSERT_EQ(v.capacity, 2);
   ASSERT_EQ(v.size, 2);
   for (int i = 0; i < 2; i++)
      EXPECT_EQ(vl[i], ((int *) prosVector_getAt(&v, i))[0]);
   // Now, the vector is full.

   prosVector_pushArray(&v, &vl[2], 6);
   ASSERT_EQ(v.capacity, 8);
   ASSERT_EQ(v.size, 8);
   for (int i = 0; i < 2; i++)
      EXPECT_EQ(vl[i], ((int *) prosVector_getAt(&v, i))[0]);
...
TEST_F(ProtocolsUtilitiesVector, MetGetAtGetsReturnsAPointerToTheElementAtTheSpecifiedIndex) {
   ...
   for (int i = 0; i < 4; i++) {
      EXPECT_EQ(*static_cast<int *>(prosVector_getAt(&v, accessOrder[i])), accessOrder[i]);
   };
}
...
TEST_F(ProtocolsUtilitiesVector, MetGetLastObjOnEmptyVectorReturnsFalse) {
   ...
   prosVector_pushBack(&v, &vl);
   ASSERT_EQ(v.capacity, PROTOCOLS_INITIAL_VECTOR_SIZE)
      << "To proceed, capacity must be a non-zero value";
   ASSERT_EQ(v.size, 1);
...
TEST_F(ProtocolsUtilitiesVector, MetGetDataReturnsAliasToVectorObjectDataThatIsContiguos) {
   v = prosVector_new(sizeof(char), nullptr);
...
TEST_F(ProtocolsUtilitiesVector, MetGetCapacityCallWithNullSelfParameterPanics) {
   EXPECT_DEATH(
      {
         [[maybe_unused]]
         uint32_t r = prosVector_getCapacity(nullptr);
      },
      ""
   );
}
