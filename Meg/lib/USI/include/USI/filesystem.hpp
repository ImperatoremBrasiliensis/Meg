...
namespace Meg::filesystem {
   ...
   public:
      ...
      public:
         ...
         bool operator==(const iterator &other) const {
            return p == other.p && pos == other.pos;
         }
      };
       ...
       */
      inline size_t get_path_size() const {
         return size;
      }
       ...
       * object in bytes.
       */
      inline size_t get_path_capacity() const {
         return capacity;
      }
   ...
   struct file {
      const std::chrono::file_clock::time_point last_mod;
      const path filepath;
      const size_t size;
      const bool isvirt;
      const struct {
   ...
   class reader {
      ...
      size_t load_page_of_index(size_t index);
      size_t next_line();
   ...
   public:
      ...
      public:
         iterator(reader *r, size_t pos):
            r(r) {
            offset = pos > r->f.size ? 0 : pos;
         }
         ...
         bool operator==(const iterator &other) const {
            return r == other.r && offset == other.offset;
         }
      };
       ...
       *
       * **UNIQUE PROPERTY**: If the @p f parameter
       *                      is a valid `file` object,
       *                      it will be loaded from disk.
       *                      Otherwise, a error log
       *                      will be generated and the
       *                      object may be defective.
       *
       * @warning This constructor doesn't whether the
       * @p f parameter is valud or not.
       *
       * @param f A reference to the file from which to
       * read. 
       */
      reader(file &f);
      ...
      /**
       * @brief Reads @p n bytes of the file to the
       * buffer pointed by the @p dest parameter.
       *
       * **PROPERTY 1**: Reads @p n bytes from the
       *                 position in the file until
       *                 the end of the file. This
       *                 returns @p n as how bytes
       *                 was actually read.
       * **PROPERTY 2**: If this method can't read
       *                 @p n bytes, it returns the
       *                 actual count of bytes read.
       *                 Normally, files with 0 bytes
       *                 of size does this method read
       *                 nothing and always return 0.
       * **PROPERTY 3**: If the file is not loaded,
       *                 this method does nothing
       *                 and returns 0.
       *
       * @param dest Pointer to the buffer to which
       * copy the bytes read.
       * @param n How many bytes to read.
       *
       * @return The actual count of bytes
       * read,
       */
      size_t read(char dest[], size_t n);

      /**
		 * @brief Reads a entire line of the file and
       * advances to the next line.
		 *
		 * **PROPERTY 1**: Reads the file until the
		 *                 one past the newline
       *                 character. Returns how
       *                 many bytes were written
       *                 into the buffer pointed
       *                 by @p dest parameter.
       * **PROPERTY 2**: If @p dest parameter is
       *                 `nullptr`, just returns
       *                 the size of the current
       *                 line.
       * **PROPERTY 3**: If the file isn't loaded,
       *                 this method does nothing
       *                 and returns zero.
       *
       * @warning If this file offset is at the
       * end, this method reads nothing and
       * always returns 0.
       *
       * @param dest Buffer where to copy the data
       * read.
       * @param bufsz The buffer size, may be zero
       * if @p dest is `nullptr`.
       *
       * @return If @p dest is not `nullptr`, returns
       * how many bytes was written into @p dest;
       * otherwise, returns the size of the current
       * line.
		 */
      size_t read_line(char dest[], size_t bufsz);

      /**
       * @breief Returns `true` if the file is
       * being loaded to the internal buffer.
       *
       * **UNIQUE PROPERTY**: Returns `true` if the
       *                      file is being loaded to
       *                      the internal buffer and
       *                      this object can read the
       *                      file; returns `false`
       *                      otherwise.
       *
       * @return `true` if the file is being loaded;
       * `false` otherwise.
       */
      bool is_loaded() const;

      /**
       * @brief Changes the file offset/cursor
       * according to @p offset parameter.
       *
       * **PROPERTY 1**: If this file is loaded, @p offset
       *                 is less than this file total size
       *                 and @p from_end is `false`, the
       *                 offset will be set as @p offset
       *                 parameter.
       * **PROPERTY 2**: If this file is loaded, @p offset
       *                 is less than this file total size
       *                 and @p from_end is `true`, this
       *                 file offset will be set as the file
       *                 total size minus the @p offset
       *                 parameter.
       * **PROPERTY 3**: If this file isn't loaded or @p offset
       *                 isn't less than the file total size,
       *                 this method does nothing and returns
       *                 `false`.
       *
       * @param offset If @p from_end is `false`, offset will
       * be the @p offset parameter; otherwise, offset will
       * be this file total size minus @p offset patameter.
       * @param from_end Defines if the offset is relative
       * from the end of the file case it's `true`.
       *
       * @return `true` successful; `false` otherwise.
       */
      bool seek(size_t offset, bool from_end);
   };

   std::expected<file, error> get_file(const path &path);

   error delfile(const path &fpath);

   error delfile(file &f);

   error makdir(std::string_view name);

   error deldir(const path &directory);
};  // namespace Meg::filesystem
