...
static void handler(int signal) {
	...
	case SIGINT:
		dbg::print(
			MEG_FATA | MEG_DEFT,
			"Application interrupted. Exiting now...\n"
		);
		exit(signal);
	case SIGABRT:
		dbg::print(MEG_FATA | MEG_ERRO, "Application aborting now!\n");
		exit(signal);
	case SIGSEGV:
		dbg::print(MEG_FATA | MEG_ERRO, "Segmentation fault. Exition now...\n");
		exit(signal);
	}
}
...
dbg::session::~session() {
	...
	if (!pers_es) {
		std::erase_if(data.e_stack, [this](const exception e) {
	...
	std::erase_if(thrd_data.s_stack, [this](session *s) {
		return s == this;
	});
}
...
dbg::session::get_exceptions(exception_code code) noexcept {
	if (!id)
		return {};
...
bool dbg::session::log(uint8_t flags, const char *msg, ...) noexcept {
	...
	va_list va;
	va_start(va, msg);
	bool r = dbg::log(MEG_PERS | flags, msg, va);
	va_end(va);
...
bool dbg::session::log(uint8_t flags, const char *msg, va_list va) noexcept {
	...
	return dbg::log(MEG_PERS | flags, msg, va);
}
...
MEG_INLINE const char *choose_prefix(uint8_t flags) {
	if (flags & MEG_ERRO)
		if (flags & MEG_FATA)
			return prefix.fer;
		else
			return prefix.err;
	else if (flags & MEG_WARN)
		return prefix.wrn;
	else if (flags & MEG_DBUG)
		return prefix.dbg;
	else if (flags & MEG_INFO)
		return prefix.inf;
	else
		return prefix.def;
}
...
bool dbg::print(uint8_t flags, const char *msg, va_list va) noexcept {
	FILE *stream = flags & MEG_FATA ? stderr : stdout;
...
bool dbg::log(uint8_t flags, const char *msg, ...) noexcept {
	...
	// If the flag `dbg::MEG_PERS` wasn't set AND debugger is not enabled, returns too.
	if (!msg || (!(flags & MEG_PERS) && !is_enabled()))
		return false;
	...
	offset += vsnprintf(buffer + offset, sizeof(buffer), msg, va);
