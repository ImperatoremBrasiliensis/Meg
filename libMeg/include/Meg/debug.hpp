...
#define MEG_SET_ALIAS(name, func, flags) \
MEG_INLINE constexpr bool name(const char* msg, auto ...va) noexcept { \
	return func(flags, msg, va...); \
}

#define MEG_DEFT (uint8_t) 0b00000000
#define MEG_ERRO (uint8_t) 0b00000001
#define MEG_WARN (uint8_t) 0b00000010
#define MEG_DBUG (uint8_t) 0b00000100
#define MEG_INFO (uint8_t) 0b00001000
#define MEG_FATA (uint8_t) 0b00010000
#define MEG_PERS (uint8_t) 0b00100000

#define MEG_RESERVE_LIMIT 4096
...
namespace Meg::dbg {
	...
	public:
		 ...
		 */
		[[nodiscard]]
		std::vector<dbg::exception>
		get_exceptions(exception_code code) noexcept;
		 ...
		 * `MEG_PERS` (persistent), `MEG_FATA` (fatal), `MEG_ERRO` (error),
		 * `MEG_DBUG` (debug), `MEG_INFO` (information) and `MEG_DEFT` (default).
		 *
		 * @details The message prefix will be printed according
		 * to @p flags parameter. Exceptionally, if `MEG_FATA` was wet 
		 * with `MEG_ERRO`, the prefix will be `Meg Fatal Error`.
		 * If `MEG_PERS` was set, the log message will always be printed,
		 * until with debugger disabled.
		 * `MEG_DEFT` makes that message prefix be only `Meg`.
		 *
		...
		MEG_SET_ALIAS(log_ferr, log, MEG_PERS | MEG_FATA | MEG_ERRO);
		MEG_SET_ALIAS(log_err, log, MEG_ERRO);
		MEG_SET_ALIAS(log_war, log, MEG_WARN);
		MEG_SET_ALIAS(log_deb, log, MEG_DBUG);
		MEG_SET_ALIAS(log_inf, log, MEG_INFO);
		MEG_SET_ALIAS(log_def, log, MEG_DEFT);

		MEG_SET_ALIAS(log_pferr, log, MEG_PERS | MEG_FATA | MEG_ERRO);
		MEG_SET_ALIAS(log_perr, log, MEG_PERS | MEG_ERRO);
		MEG_SET_ALIAS(log_pwar, log, MEG_PERS | MEG_WARN);
		MEG_SET_ALIAS(log_pdeb, log, MEG_PERS | MEG_DBUG);
		MEG_SET_ALIAS(log_pinf, log, MEG_PERS | MEG_INFO);
		MEG_SET_ALIAS(log_pdef, log, MEG_PERS | MEG_DEFT);
	};
	/**
	 ...
	 * has the `MEG_PERS` (persistence) flag, you have
	 ...
	 * @param flags Defines how function will behave
	 * and how message will be printed, possible flags are:
	 * `MEG_PERS` (persistent), `MEG_FATA` (fatal), `LUNIQUE_MEG_ERRO` (error),
	 * `MEG_DBUG` (debug), `MEG_INFO` (information) and `MEG_DEFT` (default).
	 *
	 * @details The message prefix will be printed according
	 * to @p flags parameter. Exceptionally, if `MEG_FATA` was wet 
	 ...
	 * @param flags Defines how function will behave
	 * and how message will be printed, possible flags are:
	 * `MEG_FATA` (fatal), `MEG_ERRO` (error), `MEG_DBUG` (debug),
	 * `MEG_INFO` (information) and `MEG_DEFT` (default).
	 *
	 ...
	 *
	 * @details The message prefix will be printed according
	 * to @p flags parameter. Exceptionally, if `MEG_FATA` was set 
	 * with `MEG_ERRO`, the prefix will be `Meg Fatal Error`. `MEG_FATA` 
	 * also makes that the message will be printed to `stderr`.
	 * `MEG_DEFT` makes that message prefix will be only `Meg`. 
	...
	MEG_SET_ALIAS(prt_ferr, print, MEG_PERS | MEG_FATA | MEG_ERRO);
	MEG_SET_ALIAS(prt_err, print, MEG_ERRO);
	MEG_SET_ALIAS(prt_war, print, MEG_WARN);
	MEG_SET_ALIAS(prt_deb, print, MEG_DBUG);
	MEG_SET_ALIAS(prt_inf, print, MEG_INFO);
	MEG_SET_ALIAS(prt_def, print, MEG_DEFT);
